...
- **Playwright** = launches a real browser and drives the deployed UI (see `e2e/`).

## 3. Where things live

Unlike the classic `tests/` tree, this repo **co-locates** tests next to source:

```
lib/coalesce.ts              lib/coalesce.test.ts          # unit — pure logic
hooks/use-leads.ts           hooks/use-log-attempt.test.tsx # unit — a hook
components/ui/pending-button.tsx  …/pending-button.test.tsx # integration — a component
test-utils/
├── render.tsx               # renderWithProviders() + makeTestQueryClient() + user
└── fixtures.ts              # canned, type-correct API shapes (lead detail, attempts, …)
vitest.config.mts            # Vitest + coverage config
vitest.setup.ts              # global setup: next/navigation mock + jsdom polyfills + cleanup
e2e/                         # Playwright specs (real browser) — see e2e/README.md
playwright.config.ts
```

Naming: unit/integration use `*.test.{ts,tsx}` (co-located). E2E uses `*.spec.ts`
under `e2e/` (excluded from Vitest). JSX must live in a `.tsx` file.
...
npm run test:unit      # just lib + hooks
npm run test:integration  # just components + app
...
- **Vitest config** (`vitest.config.mts`): jsdom environment, the `@/…` alias, a
  ...
  `include: **/*.test.{ts,tsx}`, and excludes `e2e/` (that's Playwright's job).
  Coverage is `v8` with text/html/lcov reporters into `./coverage`.
- **Global setup** (`vitest.setup.ts`): adds jest-dom matchers, a global
...
- **Playwright config** (`playwright.config.ts`): points at `e2e/`, runs Chromium,
...
(See `hooks/use-log-attempt.test.tsx` for the full, real example.)
...
detail.mockResolvedValue(leadDetail) // from test-utils/fixtures
```

**Reuse canned data:** import from `@/test-utils/fixtures` (`leadDetail`,
...
2. **Radix UI needs polyfills.** Radix reads `matchMedia`, `ResizeObserver`,
   ...
   `vitest.setup.ts` stubs them all — a Radix Select/Dialog test crashes without them.
